...
public class RedBlackTree<K extends Comparable<K>, D> {
	...
	public class Node {
		...
		public boolean is2Node() {
			return !m_bIsRed && (m_Left == null || !m_Left.m_bIsRed)
	...
	boolean insert(K key, D data) {
		...
		h.set(new Node(key, data), h.NODE, true);
		h.split();
	...
	public int cnt3Nodes() {
		return cnt3Nodes(m_Root);
	}
	...
	private int cnt3Nodes(Node n) {
		if(n==null)
			return 0;
		if(is3Node(n))
			return 1+cnt3Nodes(n.m_Left)+cnt3Nodes(n.m_Right);
		return cnt3Nodes(n.m_Left)+cnt3Nodes(n.m_Right);
	}

	private boolean is3Node(Node n) {
		if (n.m_Left != null && n.m_Left.m_bIsRed&&(n.m_Right==null||!n.m_Right.m_bIsRed)) {
			return true;
		}
		if (n.m_Right != null && n.m_Right.m_bIsRed&&(n.m_Left == null || !n.m_Left.m_bIsRed)) {
			return true;
		}
		return false;
	}
