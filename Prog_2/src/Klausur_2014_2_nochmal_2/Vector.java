...
public class Vector {
	...
	public static Vector merge_2(Vector vec1, Vector vec2) {
		Vector vec3 = new Vector(vec1.mObjects.length + vec2.mObjects.length);
		int i_vec1 = 0, j_vec2 = 0;
		boolean vec1_isMax = false, vec2_isMax = false;

		for (int k = 0; k < vec3.mObjects.length; k++) {
			int get_vec1 = i_vec1 < vec1.mNextFree ? vec1.get(i_vec1) : vec1.get(vec1.mNextFree - 1);
			int get_vec2 = j_vec2 < vec2.mNextFree ? vec2.get(j_vec2) : vec2.get(vec2.mNextFree - 1);
			...
			if (get_vec1 <= get_vec2 || vec2_isMax) {
				if (!vec1_isMax) {
					System.out.println(get_vec1 + " wird eingefügt.");
					vec3.add(get_vec1);
					i_vec1++;
			...
			} else if (get_vec2 <= get_vec1 || vec1_isMax) {
				if (!vec2_isMax) {
					System.out.println(get_vec2 + " wird eingefügt.");
					vec3.add(get_vec2);
					j_vec2++;
	...
	public static Vector merge_3(Vector vec1, Vector vec2) {
		Vector vec3 = new Vector(vec1.size() + vec2.size());

		boolean vec1_isMax = false;
		boolean vec2_isMax = false;

		int i_vec1 = 0;
		int i_vec2 = 0;

		for (int j = 0; j < vec3.mObjects.length; j++) {
			int getVec1 = vec1.get(i_vec1);
			int getVec2 = vec2.get(i_vec2);
			...
			if (getVec1 <= getVec2 || vec2_isMax) {
				if (!vec1_isMax) {
					vec3.add(getVec1);
					++i_vec1;
				}

			} else if (getVec2 <= getVec1 || vec1_isMax) {
				if (!vec2_isMax) {
					vec3.add(getVec2);
					++i_vec2;
				}
