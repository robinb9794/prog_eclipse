...
public class StrangeCounter_AtomicModulo {
	private final static int INCREMENTERS = 50;
	private final static int RUNS = 500;

	protected static class MyLongAtomicModulo implements CounterInterface {
	...
	protected static class Incrementer implements Runnable {
		...
		public static void main(String args[]) {
			...
			ExecutorService es = Executors.newScheduledThreadPool(INCREMENTERS);
			// ExecutorService es = Executors.newSingleThreadExecutor();

			for (int i = 0; i < INCREMENTERS; i++) {
				es.submit(new Incrementer(startLatch, endLatch, new MyLongAtomicModulo()));
			}
